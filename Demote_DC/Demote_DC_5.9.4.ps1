...
# ----- End-to-end readiness gate and SA credential fallback -----
# Assert-DemotionReadiness runs the privilege checks AND the target readiness checks as one gate
# before a demotion. If any check fails and $PromptCredentialOnReadinessFailure is set, the script
# prompts once for an SA (Domain/Enterprise Admin) credential and uses it for the steps the failed
# check(s) would block - the remote WinRM session, the AD queries, and the demotion itself. A
# credential cannot elevate the current process, so a missing local elevation token is reported but
# the SA credential still covers the remote/AD/demotion steps (the demotion runs on the target under
# that credential). Hard blocks (target unset or self, FSMO role held, last DC in the domain) are
# state issues a credential cannot resolve and always stop the gate.
$EnsureEndToEndReadiness            = $true   # $true = run the combined privilege + target readiness gate before demoting
$PromptCredentialOnReadinessFailure = $true   # $true = on any failed check, prompt for SA creds to use for the blocked steps
...
# The script is run from an account that is already a member of Domain Admins and Enterprise
# Admins, logged on locally to the management DC. These switches drive a fail-closed startup
# guard (Assert-ExecutionPrivileges) so a session that was NOT launched elevated - or that is
# running under an account lacking the required directory rights - is stopped before any
# destructive action, rather than failing partway through ntdsutil / metadata removal.
...
# ----- Demotion credential (remote second-hop fix) -----
# A remote Uninstall-ADDSDomainController must contact a surviving replication partner. Over a
# default WinRM/Kerberos session the on-target token cannot delegate to that partner (the classic
# double-hop), which surfaces as a partner-contact failure and falsely escalates a graceful
# demotion to ForceRemoval. Passing an explicit PSCredential to the cmdlet's -Credential makes it
# bind to the partner with fresh credentials (not a delegated hop), so the graceful path holds.
# A blank $DemotionCredentialUser pre-fills the current logon name in the credential prompt.
$UseExplicitDemotionCredential = $true   # $true = prompt once for the SA credential and thread it into Uninstall-ADDSDomainController -Credential
...
function Get-DemotionCredential
{
    ...
    if ($Script:FallbackCredential)
    {
        return $Script:FallbackCredential
    }
...
function Get-FallbackAdminCredential
{
    # Prompts once for an SA (Domain/Enterprise Admin) credential to use for the steps a failed
    # privilege or readiness check would otherwise block - the remote WinRM session to the target,
    # the Active Directory queries, and the demotion itself. Cached for the session and adopted as
    # the demotion credential so the partner-contact second hop uses it too. Returns $null if the
    # operator cancels. NOTE: a credential cannot elevate the current process - it addresses
    # authorization and remote-access failures, not a missing local elevation token.
    if ($Script:FallbackCredential)
    {
        return $Script:FallbackCredential
    }
    ...
    Write-InfoMsg "Prompting for an SA (Domain/Enterprise Admin) credential to use for the steps the failed check(s) would block (user hint: '$userHint')."
    $cred = Get-Credential -UserName $userHint -Message 'Enter the SA (Domain/Enterprise Admin) credential to use for the WinRM, Active Directory, and demotion steps that the failed check(s) would otherwise block.'

    if ($null -eq $cred)
    {
        Write-WarnMsg "SA credential prompt was cancelled - no fallback credential is available, so the failed steps cannot be retried with alternate credentials."
        return $null
    }

    $Script:FallbackCredential = $cred

    if (-not $Script:DemotionCredential)
    {
        $Script:DemotionCredential = $cred
    }

    Write-SuccessMsg "SA fallback credential captured for '$($cred.UserName)' (cached for this session; also adopted as the demotion credential)."
    return $cred
}

function Initialize-DemotionCredential
{
    # One-time, up-front capture of the directory credential threaded into the remote demotion
    # (Uninstall-ADDSDomainController -Credential) to defeat the WinRM double-hop from the target DC
    # to a surviving partner. Running this as the first post-privilege step means every later call
    # to Get-DemotionCredential returns the cached value, so the operator enters the credential once
    # at the start and is never prompted in the middle of a demotion or the full workflow. When
    # $UseExplicitDemotionCredential is $false nothing is captured and the remote demotion relies on
    # the session token. The credential is held only for the lifetime of the session.
    if (-not $UseExplicitDemotionCredential)
    {
        Write-InfoMsg 'Explicit demotion credential is disabled ($UseExplicitDemotionCredential = $false) - the remote demotion will rely on the session token (double-hop applies).'
        return
    }

    Write-InfoMsg 'Capturing the demotion credential now, as the first step, so no credential prompt interrupts the workflow later.'
    $cred = Get-DemotionCredential

    if ($null -eq $cred)
    {
        Write-WarnMsg 'No demotion credential was captured at startup - you will be prompted again when a demotion runs, and a partner-contact double-hop may escalate a graceful demotion to a forced removal.'
    }
}
...
{
    # Focused WinRM reachability probe for the target, optionally using an explicit SA credential.
    # The readiness gate uses this to re-test reachability after capturing a fallback credential
    # without re-running the rest of the prerequisite checks. Returns $true / $false.
    param
    (
        [hashtable]$TargetInfo,
    ...
    {
        Write-WarnMsg "WinRM probe of '$($TargetInfo.HostName)' failed: $($_.Exception.Message)"
        return $false
    }
}
...
function Assert-DemotionReadiness
{
    ...
    # checks (elevation, Domain Admins, Enterprise Admins) and the target readiness checks (target
    # set and not self, WinRM reachable, no FSMO roles, replication health, not the last DC) into a
    # single pass and reports each. If ANY check fails and $PromptCredentialOnReadinessFailure is
    # set, it prompts once for an SA credential to use for the steps the failed check(s) would block
    # - the remote WinRM session, the AD queries, and the demotion - then re-probes the credential-
    # addressable items with it. Hard blocks (target unset or self / FSMO role held / last DC) are
    # state issues a credential cannot resolve and always stop the gate. A credential also cannot
    # elevate the current process; that is reported, but the SA credential still covers the remote
    # and AD steps (the demotion runs on the target under that credential). Returns a decision
    # object - the caller proceeds only when Proceed is $true.
    param
    (
        [hashtable]$TargetInfo
    )
    ...
    if ($targetIsSelf)
    {
        Write-ErrorMsg "The target resolves to the machine running this script ('$($env:COMPUTERNAME)') - the demotion must be run remotely from a different DC. This is a hard block a credential cannot resolve."
    }
    ...
    @(
        ...
        [pscustomobject]@{ Name = 'Privileges (elevation + Domain/Enterprise Admins)';     Passed = $privilegesOk; HardBlock = $false }
        [pscustomobject]@{ Name = 'Prerequisites (target set, not self, WinRM reachable)'; Passed = $prereqOk;     HardBlock = $false }
    ...
    if ($failed.Count -eq 0)
    {
        ...
        return [pscustomobject]@{
            ...
            ReadinessOk             = $true
            HardBlock               = $false
            UsingFallbackCredential = [bool]$Script:FallbackCredential
            Checks                  = $checks
        }
    }

    Write-WarnMsg "End-to-end readiness gate: $($failed.Count) check(s) failed:"

    foreach ($c in $failed)
    {
        $tag = if ($c.HardBlock) { 'HARD BLOCK - a credential cannot resolve this' } else { 'may be addressable with an SA credential' }
        Write-WarnMsg "    [FAIL] $($c.Name) - $tag."
    }

    $hardBlocked = @($failed | Where-Object { $_.HardBlock })

    if ($hardBlocked.Count -gt 0)
    {
        Write-ErrorMsg "A HARD BLOCK check failed (FSMO role held and/or target is the last DC). These are state issues - resolve them before demoting. Not proceeding."
    ...
    if (-not $PromptCredentialOnReadinessFailure)
    {
        Write-ErrorMsg 'Credential prompting on readiness failure is disabled ($PromptCredentialOnReadinessFailure = $false) - not proceeding. Resolve the failed check(s) or enable the prompt.'
    ...
    $cred = Get-FallbackAdminCredential

    if ($null -eq $cred)
    {
        Write-ErrorMsg "No SA credential was provided - the failed step(s) cannot be retried with alternate credentials. Not proceeding."
        ...
        return [pscustomobject]@{
            ...
            ReadinessOk             = $false
            HardBlock               = $false
            UsingFallbackCredential = $false
            Checks                  = $checks
        }
    }

    $winrmOk = Test-TargetWinRm -TargetInfo $TargetInfo -Credential $cred

    if ($winrmOk)
    {
        Write-SuccessMsg "WinRM to '$($TargetInfo.HostName)' is reachable using the SA credential '$($cred.UserName)'."
    }
    else
    {
        Write-ErrorMsg "WinRM to '$($TargetInfo.HostName)' is still unreachable even with the SA credential. Not proceeding."
        ...
        return [pscustomobject]@{
            ...
            ReadinessOk             = $false
            HardBlock               = $false
            UsingFallbackCredential = $true
            Checks                  = $checks
        }
    }

    if ($RequireElevation -and -not (Test-IsElevated))
    {
        Write-WarnMsg "The current process is not elevated and a credential cannot change that. The SA credential will be used for the remote WinRM, Active Directory, and demotion steps - the demotion runs on the target under that credential - but local-only elevated actions are unavailable in this session. Re-launch elevated if you need them."
    }

    Write-SuccessMsg "Proceeding with the SA credential '$($cred.UserName)' applied to the WinRM, Active Directory, and demotion steps that the failed check(s) would have blocked."
...
function Confirm-PartnerConnectionTeardown
{
    ...
    $cleared = (($dsaPresentOn.Count -eq 0) -and ($sourcingOn.Count -eq 0) -and (@($staleConns).Count -eq 0))
    $detail  = "Reachable partners checked=$($reachable.Count); DSA still present=$($dsaPresentOn.Count); still sourcing=$($sourcingOn.Count); lingering connections=$(@($staleConns).Count); KCC forced=$forcedKcc; waited=${elapsed}s."
    ...
    return [pscustomobject]@{
        ...
        Waited         = $elapsed
        ForcedKcc      = $forcedKcc
        DsaPresentOn   = @($dsaPresentOn)
        SourcingOn     = @($sourcingOn)
...
function Invoke-RemoteDemotion
{
    ...
    if ($Script:FallbackCredential)
    {
        $icmParams['Credential'] = $Script:FallbackCredential
        Write-InfoMsg "Authenticating the WinRM session to '$($TargetInfo.HostName)' with the SA fallback credential '$($Script:FallbackCredential.UserName)'."
    }
...
function Invoke-MenuDemotion
{
    ...
    if ($EnsureEndToEndReadiness)
    {
        ...
        if (-not $gate.Proceed)
        {
            ...
            $Script:State.OverallStatus = if ($gate.HardBlock) { 'Aborted - readiness hard block (FSMO held or last DC)' } else { 'Aborted - demotion readiness checks failed' }
