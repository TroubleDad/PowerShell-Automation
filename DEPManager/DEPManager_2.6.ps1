...
# =====================================================================
# Change Log:
#   2.6 - Root cause of the "Argument types do not match"
#         (System.ArgumentException) crash was identified by the v2.5
#         diagnostic trap: it was thrown by the array subexpression
#         @($batchResults) on the return from Invoke-DEPBatch. In
#         PowerShell that exception is raised by the .NET method binder
#         when the internal "no value" sentinel (AutomationNull, which is
#         distinct from $null) is materialized through @() over a
#         List[object] built from remoting-sourced objects. This build
#         removes that path:
#           * Both returns now use a direct $batchResults.ToArray()
#             delivered comma-wrapped, instead of the @() operator.
#           * ConvertTo-DEPResult drops the remoting ETS properties
#             (PSComputerName, PSShowComputerName, RunspaceId,
#             PSSourceJobInstanceId) and normalizes any null property
#             value to "N/A", so no null sentinel survives into a typed
#             path during sort, format, export or email.
#         The v2.5 diagnostic trap is retained as a safety net.
#   2.5 - The "Argument types do not match" (System.ArgumentException)
#         terminating error persisted unchanged through the v2.4 rework,
#         which proves it is not caused by Sort-Object, by the boolean
#         remoting, or by local/remote object type mixing. This build:
#           * Installs a script-scope diagnostic trap that, on any fatal
#             terminating error, prints the exact failing line
#             (InvocationInfo.PositionMessage), the exception type, the
#             fully qualified error id and the script stack trace to both
#             the console and the log file before the script stops.
#           * Hardens every Sort-Object and unique selection so the
#             comparison keys are explicitly cast to [string], as
#             defensive insurance against any hidden comparison fault.
...
trap
{
    ...
    Write-Host " FATAL TERMINATING ERROR CAPTURED BY DIAGNOSTIC TRAP"                        -ForegroundColor Red
    Write-Host "============================================================"               -ForegroundColor Red
    Write-Host " Exception Type : $($capturedError.Exception.GetType().FullName)"            -ForegroundColor Yellow
    Write-Host " Message        : $($capturedError.Exception.Message)"                       -ForegroundColor Yellow
    Write-Host " Category       : $($capturedError.CategoryInfo.Category)"                   -ForegroundColor Yellow
    Write-Host " FQ Error Id    : $($capturedError.FullyQualifiedErrorId)"                   -ForegroundColor Yellow
    Write-Host " Target Object  : $($capturedError.TargetObject)"                            -ForegroundColor Yellow
    Write-Host "------------------------------------------------------------"               -ForegroundColor Red
    Write-Host " Position (the exact failing line):"                                         -ForegroundColor Red
    Write-Host "$($capturedError.InvocationInfo.PositionMessage)"                            -ForegroundColor Cyan
    Write-Host "------------------------------------------------------------"               -ForegroundColor Red
    Write-Host " Script Stack Trace:"                                                        -ForegroundColor Red
    Write-Host "$($capturedError.ScriptStackTrace)"                                          -ForegroundColor Cyan

    if ($capturedError.Exception.InnerException)
    {
        Write-Host "------------------------------------------------------------"           -ForegroundColor Red
        Write-Host " Inner Exception:"                                                       -ForegroundColor Red
        Write-Host " $($capturedError.Exception.InnerException.GetType().FullName): $($capturedError.Exception.InnerException.Message)" -ForegroundColor Cyan
    }
    ...
    $diagnosticLines.Add("FATAL TERMINATING ERROR CAPTURED BY DIAGNOSTIC TRAP")                                                            | Out-Null
    $diagnosticLines.Add("Exception Type : $($capturedError.Exception.GetType().FullName)")                                                | Out-Null
    $diagnosticLines.Add("Message        : $($capturedError.Exception.Message)")                                                           | Out-Null
    $diagnosticLines.Add("Category       : $($capturedError.CategoryInfo.Category)")                                                       | Out-Null
    $diagnosticLines.Add("FQ Error Id    : $($capturedError.FullyQualifiedErrorId)")                                                       | Out-Null
    $diagnosticLines.Add("Position       : $($capturedError.InvocationInfo.PositionMessage)")                                              | Out-Null
    $diagnosticLines.Add("Stack Trace    : $($capturedError.ScriptStackTrace)")                                                            | Out-Null

    if ($capturedError.Exception.InnerException)
    {
        $diagnosticLines.Add("Inner          : $($capturedError.Exception.InnerException.GetType().FullName): $($capturedError.Exception.InnerException.Message)") | Out-Null
    }

    foreach ($diagnosticLine in $diagnosticLines)
    {
        Write-Log -Message $diagnosticLine
    }
...
function ConvertTo-DEPResult
{
    ...
    $remotingPropertyNames = @("PSComputerName", "PSShowComputerName", "RunspaceId", "PSSourceJobInstanceId")

    foreach ($property in $InputObject.PSObject.Properties)
    {
        if ($remotingPropertyNames -contains $property.Name)
        {
            continue
        }
...
function Write-RunSummary
{
    ...
    $ResultsToSummarize |
        Sort-Object -Property @{ Expression = { [string]$_.Server } }, @{ Expression = { [string]$_.InvocationMode } } |
...
function New-HtmlReport
{
    ...
    foreach ($row in ($Results | Sort-Object -Property @{ Expression = { [string]$_.Server } }, @{ Expression = { [string]$_.InvocationMode } }))
    {
        $rowColor = "#ffffff"
...
$ResultsSorted = $AllResults | Sort-Object -Property @{ Expression = { [string]$_.Server } }, @{ Expression = { [string]$_.InvocationMode } }

$ResultsSorted | Export-Csv -Path $ReportPath -NoTypeInformation

$RunContext = (@($ResultsSorted | ForEach-Object { [string]$_.InvocationMode } | Select-Object -Unique) -join ", ")
