...
# Change Log
#
#   4.7.1  - Invoke-CertScan now forces $ProgressPreference = 'Continue' (function
#            -scoped) so the scan progress bar renders even when the launching
#            session inherits SilentlyContinue from a profile. Reverts on return.
...
function Invoke-CertScan
{
    ...
    # Force the progress bar on for the duration of the scan, regardless of any
    # inherited $ProgressPreference (e.g. a profile that sets SilentlyContinue).
    # This assignment is function-scoped and reverts automatically on return.
