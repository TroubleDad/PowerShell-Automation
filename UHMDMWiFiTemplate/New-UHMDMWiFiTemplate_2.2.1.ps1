...
# --- IX509EnrollmentPolicyServer::Initialize() arguments (AD-based policy) ---
# Initialize(bstrPolicyServerUrl, bstrPolicyServerId, authFlags, fIsUnTrusted, context)
# The AD-based policy ignores URL / ID; authFlags uses integrated Windows auth.
# If Kerberos is rejected on a given host, drop $PolicyAuthKerberos to 0 (X509AuthNone).
$PolicyServerUrl    = ''      # empty for the AD-based policy (no CEP web service)
$PolicyServerId     = ''      # empty for the AD-based policy
$PolicyAuthKerberos = 2       # X509EnrollmentAuthFlags::X509AuthKerberos (integrated)
$PolicyIsUntrusted  = $false  # do not accept untrusted CA certs

# --- EnrollmentTemplateProperty IDs (CertEnroll certenroll.h) ---
# Values confirmed against the EnrollmentTemplateProperty enumeration.
# KeyUsage = 23 exists on Server 2016+, which places SubjectNameFlags at 25
# (older headers that omit KeyUsage list it as 24); this host targets 25.
$TemplateProp = @{
    CommonName       = 1     # TemplatePropCommonName
    FriendlyName     = 2     # TemplatePropFriendlyName
    EKUs             = 3     # TemplatePropEKUs
    SubjectNameFlags = 25    # TemplatePropSubjectNameFlags
}
...
function Get-BaseTemplateComObject
{
    ...
    $policy.Initialize($PolicyServerUrl, $PolicyServerId, $PolicyAuthKerberos, $PolicyIsUntrusted, $EnrollContextAdminForceMachine)
