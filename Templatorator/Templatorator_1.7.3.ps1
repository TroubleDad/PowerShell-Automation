...
$DomainConfigs = @{
    'uhhs.com' = [pscustomobject]@{
        ...
        DnsName     = 'uhhs.com'
        NetbiosName = 'UHHS'
        # DcName: when set, this specific DC FQDN is used as the LDAP server
        # component instead of the domain DNS name. Binding to a named DC is far
        # more reliable under runas /netonly, where the DC locator otherwise runs
        # in the local machine context and can fail to find a DC in the target
        # forest. Leave '' for the home forest (the locator works there).
        DcName      = ''
        CAConfigs  = @(
...
function Get-LdapServerPrefix
{
    ...
    # the bind is directed at the selected forest (e.g. LDAP://tddc01.uhtd.local/
    # CN=...) instead of the machine's joined domain. A specific DC FQDN (DcName)
    # is preferred when configured because, under runas /netonly, binding to a
    # named server is reliable where a domain-name bind can fail in the DC
    # locator. Falls back to the forest DNS name when DcName is blank. Returns
    # the server with a trailing slash, ready to prepend to a DN.
    $domain = Get-ActiveDomain

    $server = $domain.DnsName

    if (($null -ne $domain.PSObject.Properties['DcName']) -and
        (-not [string]::IsNullOrWhiteSpace($domain.DcName)))
    {
        $server = $domain.DcName
    }
...
function Invoke-Main
{
    ...
    # An explicit -DomainKey on the command line means this process IS the
    # relaunched session (started by runas /netonly with the target forest's
    # network credentials). Under /netonly the LOCAL identity is unchanged, so
    # Test-DomainCredentialNeeded would still report this forest as foreign and
    # Set-ActiveDomain would try to relaunch AGAIN - an endless loop of windows.
    # Set the active forest directly here instead, trusting the flag, and bind
    # with the network credentials runas already supplied.
    if (-not [string]::IsNullOrWhiteSpace($DomainKey))
    {
        ...
        $forced = Get-ActiveDomain
        Write-InfoMsg -Message ('Forest preset to [{0}] ({1}) from -DomainKey; using the network credentials this session was started with.' -f $forced.Label, $forced.DnsName)
    }
