...
#               forest), and network access to smtp.uhhs.com:25. When a
#               non-default forest is selected from an account in a different
#               forest, credentials valid in the target forest are prompted
#               for and applied to LDAP binds; publishing, enrollment-rights
#               grants, and SID resolution still require running as a target-
#               forest account (token-bound), so a trust must permit the
#               cross-forest LDAP bind.
#
# Change Log:#
...
function Get-DomainCredential
{
    ...
    # Returns the cached PSCredential for the forest, or $null when the running
    # account already belongs to that forest (no credential required). The first
    # time a foreign forest is used, the operator is prompted once and the
    # credential is cached on $Script:DomainCredentials for the session.
    if (-not (Test-DomainCredentialNeeded -DomainKey $DomainKey))
    {
        return $null
    }

    if ($Script:DomainCredentials.ContainsKey($DomainKey) -and
        ($null -ne $Script:DomainCredentials[$DomainKey]))
    {
        return $Script:DomainCredentials[$DomainKey]
    }

    $target = $DomainConfigs[$DomainKey]

    Write-InfoMsg -Message ('Forest [{0}] differs from the script account''s forest; credentials are required.' -f $target.Label)

    $promptMessage = 'Enter credentials valid in {0} (e.g. {1}\sAMAccountName).' -f $target.DnsName, $target.NetbiosName
    $cred          = $Host.UI.PromptForCredential('Credentials for ' + $target.Label, $promptMessage, '', $target.NetbiosName)

    if ($null -eq $cred)
    {
        throw ('Credentials are required to work in forest [{0}] from this account.' -f $target.Label)
    }

    $Script:DomainCredentials[$DomainKey] = $cred
    Write-SuccessMsg -Message ('Captured credentials [{0}] for forest [{1}].' -f $cred.UserName, $target.Label)

    return $cred
}

function Get-ActiveDomainCredential
{
    [CmdletBinding()]
    param()

    # Convenience wrapper: the credential (or $null) for the currently active
    # forest. Used by the DirectoryEntry factory on every bind.
    return (Get-DomainCredential -DomainKey $Script:ActiveDomainKey)
}
...
function New-DomainDirectoryEntry
{
    ...
    # Single factory for every LDAP bind. When the active forest requires
    # credentials, the DirectoryEntry is constructed with the cached user name
    # and password (AuthenticationTypes Secure, the default) so the bind
    # authenticates across the trust. When no credential is needed (the account
    # is already in the target forest), the serverless-token constructor is used.
    $cred = Get-ActiveDomainCredential
    ...
    # AuthenticationType Secure (Kerberos/NTLM negotiate) plus Sealing makes the
    # cross-forest credentialed bind deterministic; the default flags can leave
    # a credentialed bind unsigned and the lazy connect silently empty.
...
function Set-ActiveDomain
{
    ...
    # Capture target-forest credentials now (prompts once) when the script
    # account is not in this forest, so the rest of the session binds with them.
    # A foreign forest also means the token-bound PSPKI / grant steps will
    # defer; warn up front so the operator knows publish/grants need that
    # account's own shell.
    if (Test-DomainCredentialNeeded -DomainKey $DomainKey)
    {
        [void](Get-DomainCredential -DomainKey $DomainKey)
        Write-WarnMsg -Message ('Template authoring in [{0}] will use the supplied credentials, but publishing to its CAs, enrollment-rights grants, and principal resolution run under the process token and will be deferred. Perform those from a {1} account shell.' -f $domain.Label, $domain.NetbiosName)
    }
}
...
function Select-Domain
{
    ...
    Set-ActiveDomain -DomainKey $selected
}
...
function Invoke-Main
{
    ...
    {
        Set-ActiveDomain -DomainKey $DomainKey
    }
