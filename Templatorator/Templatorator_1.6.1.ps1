...
function Restart-UnderDomainCredential
{
    ...
    # The program runas will start, as a single string: host + switches + encoded
    # command. -NoExit keeps the relaunched window open for the interactive menu.
    $runCommand = '"{0}" -NoExit -ExecutionPolicy Bypass -EncodedCommand {1}' -f $hostPath, $encoded

    Write-InfoMsg -Message ('Relaunching {0} as [{1}] in forest [{2}] via runas /netonly. Enter the password for {1} at the prompt below.' -f $ScriptName, $userName, $target.Label)

    try
    {
        # /netonly: use these credentials for network access only. runas will
        # prompt for the password on this console, then start the new window.
        $runasArgs = @('/netonly', ('/user:{0}' -f $userName), $runCommand)
        $proc      = Start-Process -FilePath 'runas.exe' -ArgumentList $runasArgs -WorkingDirectory $ScriptRoot -Wait -PassThru -NoNewWindow
    }
    catch
    {
    ...
    {
        # runas returns non-zero when the credential prompt is cancelled or the
        # logon is refused. Stay on the current forest rather than exiting.
        Write-WarnMsg -Message ('runas did not start the relaunched session (exit code {0}). Common causes: the password prompt was cancelled, the account is locked or disabled, or {1} could not be reached. Remaining on the current forest.' -f $proc.ExitCode, $target.DnsName)
        return $false
    }
