...
function Invoke-DelegatePublishRights
{
    ...
    $Script:RunContext = [pscustomobject]@{
        ...
        Summary = @(
            @('Mode', 'Delegate publish rights')
            @('Principal', $principal)
            @('Issuing CA', $CAConfig)
            @('Permission', 'Write certificateTemplates (Enrollment Services object)')
            @('Run By', ('{0}\{1} on {2}' -f $env:USERDOMAIN, $env:USERNAME, $env:COMPUTERNAME))
        )
    }
...
function Invoke-MainMenu
{
    ...
    {
        Write-Host ''
        Write-Host '==== UH Certificate Template Tool ====' -ForegroundColor Cyan
        Write-Host '  1. Create a certificate template'
        Write-Host '  2. Modify a certificate template'
        Write-Host '  3. Delete a certificate template'
        Write-Host '  Q. Exit'
        ...
        switch ($choice.Trim().ToUpper())
        {
            ...
            'Q' {
                $Script:RunContext = [pscustomobject]@{
                    ...
                    Summary = @(
                        @('Operations', [string]$opCount)
                        @('Engine', 'Direct LDAP (Configuration partition) + PSPKI')
                        @('Run By', ('{0}\{1} on {2}' -f $env:USERDOMAIN, $env:USERNAME, $env:COMPUTERNAME))
                    )
                }
