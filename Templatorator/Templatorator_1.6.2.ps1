...
#               window) so every operation - LDAP, the PSPKI publish/grant
#               cmdlets, and SID translation - runs under the correct token.
#
...
#               use, so a wrong password surfaces in the relaunched window.
#
# Change Log:#
...
function Test-DomainCredentialNeeded
{
    ...
    # Credentials are needed when the target forest differs from the forest the
    # script's own account belongs to. The current account's domain is read from
    # $env:USERDNSDOMAIN (FQDN, e.g. UHHS.COM) with $env:USERDOMAIN (NetBIOS) as
    # a fallback, and compared case-insensitively against the target's DnsName
    # and NetbiosName. When the account is already in the target forest, no
    # prompt is required and the process token is used directly.
    ...
    if (-not [string]::IsNullOrWhiteSpace($currentDns) -and
        ($currentDns -ieq $target.DnsName))
    {
        return $false
    }

    if (-not [string]::IsNullOrWhiteSpace($currentNetbios) -and
        ($currentNetbios -ieq $target.NetbiosName))
    {
        return $false
    }
...
function Get-ScriptHostPath
{
    ...
    # Path to the host executable that is running this script, so the relaunch
    # uses the same host (Windows PowerShell powershell.exe vs PowerShell 7
    # pwsh.exe). Falls back to powershell.exe when the process path cannot be
    # read.
    try
    {
    ...
    {
        $hostPath = Join-Path -Path $PSHOME -ChildPath 'powershell.exe'
    }
...
function Restart-UnderDomainCredential
{
    ...
    # Relaunches the entire script as an account in the target forest so every
    # operation - LDAP binds, the PSPKI publish/grant cmdlets, and NTAccount SID
    # translation - runs with that forest's credentials. This uses runas /netonly
    # (LOGON32_LOGON_NEW_CREDENTIALS): the local interactive session is unchanged,
    # but all NETWORK authentication (LDAP, RPC to the CAs) is performed as the
    # supplied account. Unlike Start-Process -Credential, which does an INTERACTIVE
    # logon and therefore requires the foreign account to hold 'Allow log on
    # locally' on this workstation (it does not, which is why that approach failed
    # with a misleading 'user name or password is incorrect'), /netonly needs only
    # the correct password and a reachable target forest. runas prompts for the
    # password itself on this console; it cannot accept a captured PSCredential.
    $target   = $DomainConfigs[$DomainKey]
    ...
    # The account to authenticate as on the network, in DOMAIN\user form. Default
    # to the target's NetBIOS name and let the operator complete / correct it.
    ...
    # Build the inner command the relaunched shell will run. -DomainKey preselects
    # the forest and -DryRun preserves the current posture. Single quotes around
    # the path / string args, with embedded single quotes doubled.
    ...
    # Pass the inner command as -EncodedCommand (Base64 of UTF-16LE) so it crosses
    # the runas argument boundary without any nested-quoting problems.
    $encoded = [Convert]::ToBase64String([System.Text.Encoding]::Unicode.GetBytes($innerCommand))

    # The program runas will start, as ONE argument: host + switches + encoded
    # command. Use the bare host EXE name (powershell.exe / pwsh.exe), which is
    # resolvable via PATH and contains no spaces, so this string needs no inner
    # quotes - critical, because embedded quotes inside a single argument get
    # mangled when PowerShell passes it on to runas.exe. The Base64 -Encoded
    # Command has no spaces either, so the whole string is quote-free. -NoExit
    # keeps the relaunched window open for the interactive menu.
    $hostExe    = Split-Path -Path $hostPath -Leaf
    $runCommand = '{0} -NoExit -ExecutionPolicy Bypass -EncodedCommand {1}' -f $hostExe, $encoded

    Write-InfoMsg -Message ('Relaunching {0} as [{1}] in forest [{2}] via runas /netonly.' -f $ScriptName, $userName, $target.Label)
    Write-Host ''
    Write-Host ('  You will now be prompted by Windows for the password for [{0}].' -f $userName) -ForegroundColor Yellow
    Write-Host '  A new window will open as that account once the password is accepted.' -ForegroundColor Yellow
    Write-Host ''

    # Invoke runas.exe DIRECTLY (call operator), not through Start-Process.
    # runas needs the live, attached console to display its own secure password
    # prompt; launching it via Start-Process (even -NoNewWindow) detaches that
    # console and runas exits 1 immediately without ever prompting. Called
    # directly it inherits this console, prompts normally, and starts the inner
    # powershell in its own new window. $LASTEXITCODE carries runas's result.
    try
    {
        & runas.exe '/netonly' ('/user:{0}' -f $userName) $runCommand
        $runasExit = $LASTEXITCODE
    }
    catch
    {
        throw ('Failed to invoke runas for [{0}] in forest [{1}]: {2}.' -f $userName, $target.Label, $_.Exception.Message)
    }

    if ($runasExit -ne 0)
    {
        # Non-zero: the password prompt was cancelled, or runas refused to launch
        # (locked/disabled account, bad user name format). Note /netonly does NOT
        # validate the password here - a wrong password still returns 0 and the
        # failure surfaces in the relaunched window on first network use.
        Write-WarnMsg -Message ('runas did not start the relaunched session (exit code {0}). Likely the password prompt was cancelled, or the account is locked/disabled or the user name was not accepted. Remaining on the current forest.' -f $runasExit)
        return $false
    }

    Write-SuccessMsg -Message ('A new {0} window is starting as [{1}] for forest [{2}]. This window will now close.' -f $ScriptName, $userName, $target.Label)
...
function Get-RootDseAttribute
{
    ...
    {
        throw ('Unable to bind RootDSE for forest [{0}] at [{1}] and read [{2}]: {3}. Verify the forest DNS name resolves, a DC is reachable, and the supplied credentials are valid in that forest.' -f $domain.Label, (Get-RootDsePath), $AttributeName, $_.Exception.Message)
    }
...
function Test-ActiveForestIsForeign
{
    ...
    # True when the active forest is not the script account's forest. Token-bound
    # operations (PSPKI publish/unpublish, the Read + Enroll grant, NTAccount SID
    # translation) use this to defer with a clear WARN instead of failing under
    # the wrong identity.
...
function Get-LdapServerPrefix
{
    ...
    # the bind is directed at the selected forest (e.g. LDAP://uhtd.local/CN=...)
    # instead of the machine's joined domain. Returns the forest DNS name with a
    # trailing slash, ready to prepend to a DN.
    $domain = Get-ActiveDomain

    return ('{0}/' -f $domain.DnsName)
}
...
function Read-PagedMenuChoice
{
    ...
    while ($true)
    {
        ...
        Write-Host '  Enter a listed number, or N / P to page through the list.' -ForegroundColor Yellow
    }
}
...
function Select-BaseTemplate
{
    ...
    $choice   = Read-PagedMenuChoice -Title 'Select the base template to clone from:' -Items $labels
...
function Get-PublishingCANames
{
    ...
    # Read-only: returns the display label of every CA configured for the
    # active forest whose certificateTemplates currently lists the template.
    # The configured CA list (not an ambient Get-CertificationAuthority sweep)
    # is used so the preview stays scoped to the selected forest and is correct
    # across a forest boundary. Any CA that cannot be read is skipped.
...
function Invoke-MainMenu
{
    ...
    while ($true)
    {
        ...
        switch ($choice.Trim().ToUpper())
        {
            ...
            default {
                Write-Host '  Please choose D, 1, 2, 3, or Q.' -ForegroundColor Yellow
            }
        }
    }
}
...
function Invoke-Main
{
    ...
    # Honor an explicit -DomainKey (used by the non-interactive delegate mode,
    # and as an initial selection for the menu). Switching captures any required
    # cross-forest credentials up front.
    if (-not [string]::IsNullOrWhiteSpace($DomainKey) -and ($DomainKey -ne $Script:ActiveDomainKey))
    {
        $switchResult = Set-ActiveDomain -DomainKey $DomainKey

        if ($switchResult -eq 'Relaunched')
        {
            return ('Relaunched into forest [{0}] as a different account' -f $DomainKey)
        }
    }
