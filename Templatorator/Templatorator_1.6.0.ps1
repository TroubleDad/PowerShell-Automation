...
#               script via Start-Process -Credential as an account in that
#               forest; that account must be able to log on to this computer
#               and hold the needed AD CS rights (Start-Process -Credential
#               cannot also elevate, so no separate UAC step is available).
#
# Change Log:#
...
function Restart-UnderDomainCredential
{
    ...
    # translation - runs under a token native to that forest. This is the only
    # model where publishing and enrollment grants actually succeed cross-forest;
    # in-process credentialed binds cannot carry the token to PSPKI. The new
    # process is started with Start-Process -Credential (a full interactive logon
    # as the supplied account), -NoExit to keep its window open for the menu, and
    # the forest preselected via -DomainKey so it does not prompt again. Note:
    # -Credential cannot be combined with elevation (-Verb RunAs), so the target
    # account must already hold the rights it needs without a separate UAC step.
    $target   = $DomainConfigs[$DomainKey]
    ...
    $cred = $Host.UI.PromptForCredential(
        ('Credentials for ' + $target.Label),
        ('Enter credentials for an account in {0} (e.g. {1}\sAMAccountName). The script will relaunch as this account.' -f $target.DnsName, $target.NetbiosName),
        '',
        $target.NetbiosName)

    if ($null -eq $cred)
    {
        Write-WarnMsg -Message ('Forest switch to [{0}] cancelled - no credentials supplied.' -f $target.Label)
        return $false
    }

    # Forward the run mode via -Command (not -File): powershell.exe -File passes
    # $-prefixed tokens as literal strings, so a [bool] -DryRun would bind from
    # the string '$True' and misbehave (every non-empty string is $true). Built
    # as a -Command call, $true/$false are evaluated by the child shell and bind
    # correctly, preserving the current DryRun posture across the relaunch. The
    # script path and string args are single-quoted; embedded single quotes are
    # doubled to keep the command string safe.
    $dryRunToken = if ($DryRun) { '$true' } else { '$false' }

    $safeFile   = $filePath -replace "'", "''"
    $safeDomain = $DomainKey -replace "'", "''"

    $command = "& '{0}' -DomainKey '{1}' -DryRun {2}" -f $safeFile, $safeDomain, $dryRunToken

    if ($DelegatePublishRights)
    {
        $command += ' -DelegatePublishRights'

        if (-not [string]::IsNullOrWhiteSpace($DelegateToPrincipal))
        {
            $safePrincipal = $DelegateToPrincipal -replace "'", "''"
            $command      += " -DelegateToPrincipal '{0}'" -f $safePrincipal
        }
    }
    ...
    Write-InfoMsg -Message ('Relaunching {0} as [{1}] in forest [{2}].' -f $ScriptName, $cred.UserName, $target.Label)
    ...
    {
        throw ('Failed to relaunch as [{0}] in forest [{1}]: {2}. Verify the account name and password, that the account may log on to this computer, and that {3} is reachable.' -f $cred.UserName, $target.Label, $_.Exception.Message, $target.DnsName)
    }

    Write-SuccessMsg -Message ('A new {0} window is opening as [{1}] for forest [{2}]. This window will now close.' -f $ScriptName, $cred.UserName, $target.Label)
