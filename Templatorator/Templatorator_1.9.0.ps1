...
#   1.8.6 - 2026-06-22 - Fixed a strict-mode crash ('The property Count cannot
#                        be found on this object') that aborted the Create flow
#                        right after the enrollment grant was entered. With no
#                        per-action try/catch around Invoke-InteractiveCreate
#                        the throw propagated to the top-level handler, which
#                        logged an ERROR, wrote a ledger entry, and the finally
#                        block then sent the report - hence the ERROR-then-send
#                        seen in the console. The three Create-path reads of the
#                        grants collection (Show-DefinitionSummary plus the two
#                        Invoke-InteractiveCreate sites) called .Count directly
#                        on $Definition.Grants / $def.Grants; under Set-Strict
#                        Mode -Version Latest a $null has no Count member, so a
#                        grants value reaching the read as $null threw. Each
#                        read is now coerced with @(...).Count, the same idiom
#                        used elsewhere (Get-DomainCAConfigs, the eku change),
#                        so the count is correct whether the grants arrive as
#                        $null, a single object, or an array. Same strict-mode
#                        family as 1.3.9, which fixed the producer side (Read-
#                        EnrollmentGrants returning ,$grants.ToArray()); this
#                        closes the class at the point of use. The Modify path
#                        was reviewed and is unaffected: its .Count reads are on
#                        the $changes hashtable and a locally-built $grants
#                        collection, both of which always expose .Count.
#   1.8.7 - 2026-06-22 - Added precise fault-location logging to the top-level
#                        catch after a report that the 1.8.6 grants guards did
#                        not resolve the strict-mode 'Count' crash. The handler
#                        previously logged only $_.Exception.Message, with no
#                        line or call site, so the throwing statement could not
#                        be confirmed from the log alone. It now also logs the
#                        originating line number, the trimmed source text of
#                        that line ($_.InvocationInfo), and each frame of
#                        $_.ScriptStackTrace - all through Write-ErrorMsg so the
#                        detail lands in both the console and the log file. The
#                        ledger detail and mode result carry the line number
#                        too. No control flow changed; this is diagnostic only,
#                        and the 1.8.6 @(...) guards are retained. On the next
#                        failing run the log will name the exact statement that
#                        throws, removing the remaining guesswork.
#   1.8.8 - 2026-06-22 - Root-caused and fixed the strict-mode 'Count' crash
#                        the 1.8.7 diagnostics pinned to Resolve-ActiveCAConfig
#                        line 1432 ($configs.Count), reached from the create
#                        flow's publish step. The fault was in Get-DomainCA
#                        Configs, which did 'return @($domain.CAConfigs)': the
#                        @() builds an array inside the function but a bare
#                        return lets the output pipeline unroll it, so the
#                        caller received a bare string for a single-CA forest
#                        (and $null for an empty list), not an array - and
#                        $configs.Count then threw under Set-StrictMode. UHHS
#                        defines exactly one issuing CA, so it hit this every
#                        time; a two-CA forest happened to survive because two
#                        emitted strings re-collect into an array. The return
#                        is now ',@($domain.CAConfigs)' so the array is kept
#                        intact across the return for 0, 1, or N CAs, the same
#                        unroll-guard idiom as Read-EnrollmentGrants. This one
#                        line corrects every caller (Resolve-ActiveCAConfig and
#                        the publish/unpublish sites). The 1.8.6 grants guards
#                        and the 1.8.7 fault-location logging are retained.
#   1.8.9 - 2026-06-22 - Made the Create report Summary list each operator
#                        answer in the order it was asked, so a reviewer can
#                        see exactly what was chosen. Two answers previously
#                        survived only in machine form: EKU was stored as raw
#                        OIDs and the subject source as a numeric name flag, so
#                        the email showed '1.3.6.1.5.5.7.3.2, ...' and a hex
#                        value instead of the menu labels. Read-TemplateDefini
#                        tion now captures the selected menu label for both
#                        (EkuChoiceText, SubjectChoiceText) on the definition
#                        object; the summary rows show those labels (EKU with
#                        the OIDs appended in brackets) and are relabelled and
#                        reordered to read as a transcript - Forest, Base
#                        Template, New Template (CN), Display Name, Extended Key
#                        Usage, Subject Name Source, Minimum Key Size, each
#                        Enrollment Grant with its rights, Publish to Issuing
#                        CA, DryRun, Engine, Run By. Show-DefinitionSummary (the
#                        console preview) uses the same friendly labels so the
#                        console and the email agree. Behaviour is unchanged for
#                        Modify and Delete.
#   1.9.0 - 2026-06-22 - Made the emailed report carry each operation's full
#                        detail, not just the session header. The 1.8.9 answer
#                        rows were being discarded: on exit the menu overwrote
#                        $Script:RunContext with a session-only summary
#                        (Operations, Forest, DryRun, Engine, Run By), so the
#                        per-operation answers never reached the email. Added a
#                        session-wide detail log ($Script:SessionDetails) and a
#                        helper, Add-SessionOperation, that every mode now calls
#                        in place of setting RunContext directly. The helper
#                        still sets RunContext (the report's fallback if the
#                        session ends abnormally mid-operation) and also appends
#                        a titled section - a row keyed '__SECTION__' followed
#                        by that operation's rows - to the session log. On exit
#                        the report summary is the session header followed by
#                        every recorded section, so a reviewer sees the answers
#                        for each Create, Modify, Delete, and Delegate performed
#                        in the session. Build-HtmlReport renders a '__SECTION__'
#                        row as a full-width sub-heading. The session header drops
#                        the now-redundant Engine row (each section carries it).
# =====================================================================
...
# --- Session-wide detail log: each operation appends a titled section here so
#     the end-of-session report lists every operation's answers, not just a
#     header. A row with key '__SECTION__' marks the start of an operation. ---
$Script:SessionDetails = New-Object System.Collections.Generic.List[object]
...
function Get-DomainCAConfigs
{
    ...
    # The unary comma is REQUIRED: a bare 'return @(...)' lets the output
    # pipeline unroll the array, so a single-CA forest would hand the caller a
    # bare string and an empty list would hand back $null - and the caller's
    # $configs.Count then throws 'property Count cannot be found' under strict
    # mode. The comma keeps the array intact across the return for 0, 1, or N
    # CAs (same idiom as Read-EnrollmentGrants' ,$grants.ToArray()).
    $domain = Get-ActiveDomain
...
function Read-TemplateDefinition
{
    ...
    $ekuChoice     = Read-MenuChoice -Title 'Extended Key Usage:' -Items $ekuItems
    $ekuChoiceText = $ekuItems[$ekuChoice - 1]
    ...
    $nameChoice     = Read-MenuChoice -Title 'Subject name source:' -Items $nameItems
    $nameChoiceText = $nameItems[$nameChoice - 1]
    ...
    return [pscustomobject]@{
        ...
        EkuList           = $ekuList
        EkuChoiceText     = $ekuChoiceText
        NameFlag          = $nameFlag
        SubjectChoiceText = $nameChoiceText
        MinKeySize        = $minKey
        Grants            = $grants
        Publish           = $publish
    }
...
function Show-DefinitionSummary
{
    ...
    {
        $nameText = '{0} (0x{1:X8})' -f $Definition.SubjectChoiceText, $Definition.NameFlag
    }
    else
    {
        $nameText = $Definition.SubjectChoiceText
    }
    ...
    Write-Host ('  EKU           : {0} [{1}]' -f $Definition.EkuChoiceText, ($Definition.EkuList -join ', '))
...
function Add-SessionOperation
{
    ...
    # Records one completed operation two ways. First as the current RunContext,
    # which the report falls back to if the session ends abnormally mid-operation
    # (the top-level catch sends the report before the exit summary is built).
    # Second as a titled section appended to the session-wide detail log, so the
    # normal end-of-session report can list every operation's answers in full.
    $Script:RunContext = [pscustomobject]@{
        Heading = $Heading
        Summary = $Rows
    }

    $Script:SessionDetails.Add(@('__SECTION__', $Heading))

    foreach ($row in $Rows)
    {
        $Script:SessionDetails.Add($row)
    }
}
...
function Build-HtmlReport
{
    ...
    if ($null -ne $Script:RunContext)
    {
        ...
        {
            if ($pair[0] -eq '__SECTION__')
            {
                $summaryRows += ('<tr><td colspan="2" style="padding:14px 12px 4px 12px;font-weight:700;color:#0b3d6e;border-top:1px solid #d0d7de;">{0}</td></tr>' -f $pair[1])
            }
            else
            {
                $summaryRows += ('<tr><td style="padding:4px 12px;color:#666;vertical-align:top;">{0}</td><td style="padding:4px 12px;font-weight:600;">{1}</td></tr>' -f $pair[0], $pair[1])
            }
        }
    }
...
function Invoke-ModifyTemplate
{
    ...
    Add-SessionOperation -Heading 'Modify Certificate Template' -Rows $modSummary.ToArray()
...
function Invoke-DeleteTemplate
{
    ...
    Add-SessionOperation -Heading 'Delete Certificate Template' -Rows $delSummary.ToArray()
...
function Invoke-InteractiveCreate
{
    ...
    # The summary mirrors the operator's answers in the order they were asked so
    # the report reads as a record of exactly what was chosen. EKU and subject
    # carry the menu label that was selected; the EKU OIDs are appended in
    # brackets for the technical reader.
    $summary = New-Object System.Collections.Generic.List[object]
    $summary.Add(@('Forest', (Get-ActiveDomain).Label))
    $summary.Add(@('Base Template', $def.BaseName))
    $summary.Add(@('New Template (CN)', $def.ShortName))
    $summary.Add(@('Display Name', $def.DisplayName))
    $summary.Add(@('Extended Key Usage', ('{0} [{1}]' -f $def.EkuChoiceText, ($def.EkuList -join ', '))))
    $summary.Add(@('Subject Name Source', $def.SubjectChoiceText))
    $summary.Add(@('Minimum Key Size', [string]$def.MinKeySize))

    if (@($def.Grants).Count -eq 0)
    {
        $summary.Add(@('Enrollment Grants', 'None'))
    }
    else
    {
        foreach ($grant in $def.Grants)
        {
            ...
            $summary.Add(@('Enrollment Grant', ('{0} ({1})' -f $grant.Principal, $rightsText)))
        }
    }

    $summary.Add(@('Publish to Issuing CA', $publishText))
    ...
    Add-SessionOperation -Heading 'Create Certificate Template' -Rows $summary.ToArray()
...
function Invoke-DelegatePublishRights
{
    ...
    Add-SessionOperation -Heading 'Delegate Publish Rights' -Rows $delegateSummary.ToArray()
    ...
    while ($true)
    {
        ...
        switch ($choice.Trim().ToUpper())
        {
            ...
            'Q' {
                ...
                # Append every operation's recorded section so the report lists
                # the full set of answers, not just the session header.
                foreach ($row in $Script:SessionDetails)
                {
                    $exitSummary.Add($row)
                }
...
{
    # Capture the precise fault location so any recurrence is pinned to an exact
    # line and statement instead of a bare message. PowerShell preserves the
    # ORIGINAL throw site in the ErrorRecord, so InvocationInfo here is the line
    # that actually threw (not this handler); ScriptStackTrace is the call chain
    # that reached it. Every line below routes through Write-ErrorMsg, so the
    # location and stack land in both the console and the log file.
    $faultLine   = $_.InvocationInfo.ScriptLineNumber
    $faultSource = ([string]$_.InvocationInfo.Line).Trim()
    $faultStack  = [string]$_.ScriptStackTrace

    Write-ErrorMsg -Message $_.Exception.Message
    Write-ErrorMsg -Message ('Fault at line {0}: {1}' -f $faultLine, $faultSource)

    foreach ($frame in ($faultStack -split '[\r\n]+'))
    {
        if (-not [string]::IsNullOrWhiteSpace($frame))
        {
            Write-ErrorMsg -Message ('  stack: {0}' -f $frame.Trim())
        }
    }

    Add-LedgerEntry -Action 'Run' -Result 'Skipped' -Detail ('{0} (line {1}: {2})' -f $_.Exception.Message, $faultLine, $faultSource)
    $modeResult = ('Error: {0} (line {1})' -f $_.Exception.Message, $faultLine)
}
finally
{
