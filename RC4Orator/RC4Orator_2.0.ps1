...
#                        structure.
#   2.0.0 - 06-18-2026 - Added support for processing users in a list
#                        C:\Temp\RC4Accounts.txt
# =====================================================================
...
# --- Optional input list ----------------------------------------------
$InputListPath  = 'C:\Temp\RC4Accounts.txt'   # One SamAccountName per line.
$UseInputList   = $true                       # $true = use list, $false = use OU query

# --- Behavior toggles -------------------------------------------------
$DryRun          = $true      # $true = report only; set $false to apply changes.
...
function Get-UsersFromList
{
    ...
    {
        Write-ErrorMsg -Message ('Input list not found: {0}' -f $Path)
        return @()
    }

    $Names = Get-Content -Path $Path | Where-Object { -not [string]::IsNullOrWhiteSpace($_) }

    if (@($Names).Count -eq 0)
    {
        Write-WarnMsg -Message ('Input list {0} is empty' -f $Path)
        return @()
    }

    Write-InfoMsg -Message ('Loaded {0} entries from input list' -f $Names.Count)
    ...
    foreach ($Name in $Names)
    {
        try
        {
            $User = Get-ADUser -Identity $Name -Properties @('msDS-SupportedEncryptionTypes', 'DistinguishedName', 'Enabled') -ErrorAction Stop
            $Users += $User
        }
        catch
        {
            Write-WarnMsg -Message ('{0} - not found in AD ({1})' -f $Name, $_.Exception.Message)
...
if (Import-ADModule)
{
    try
    {
        if ($UseInputList)
        {
            Write-InfoMsg -Message ('Using input list: {0}' -f $InputListPath)

            $Users = Get-UsersFromList -Path $InputListPath

            # Split valid ADUser objects vs failed lookups
            $ValidUsers   = $Users | Where-Object { $_ -is [Microsoft.ActiveDirectory.Management.ADUser] }
            $LookupErrors = $Users | Where-Object { $_ -isnot [Microsoft.ActiveDirectory.Management.ADUser] }

            foreach ($Err in $LookupErrors)
            {
                $Results += $Err
            }

            Write-InfoMsg -Message ('Resolved {0} valid AD user(s) from list' -f $ValidUsers.Count)
        }
else
{
    $ValidUsers = Get-TargetUsers
    Write-InfoMsg -Message ('Retrieved {0} user object(s) from the target OU' -f @($ValidUsers).Count)
}
