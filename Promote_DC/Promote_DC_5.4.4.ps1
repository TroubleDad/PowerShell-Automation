...
#
# Change Log:
#               5.4.4 - Added a "Replication Topology Speed-Up" section to the
#                       HTML report with a "Replication partners" row that
#                       surfaces $Script:ReplicationPartnerSummary (the outcome
#                       of the optional 5.4.3 speed-up step). Build-ReportHtml
#                       reads the script-scope summary directly, so no report
#                       parameter or Send-FinalReport call site changed. Row is
#                       color-coded: green when partners are present, amber when
#                       none are reported yet or could not be read, grey when
#                       the step was not performed / declined / a dry run.
#               5.4.3 - Added an optional, opt-in post-promotion step that
#                       speeds up creation of the new DC's replication partners
#                       (connection objects) instead of waiting for the next
#                       KCC pass. New REPLICATION TOPOLOGY region adds
#                       Invoke-ReplicationTopologySpeedup and helpers:
#                       Assert-ReplicationSpeedupReady validates that this
#                       session already holds the rights the operations need
#                       (the elevated Domain Admins + Enterprise Admins gate
#                       from Assert-ExecutionPrivileges covers repadmin /kcc and
#                       /syncall; the validated promotion credential covers the
#                       remote Netlogon restart on the target), and that
#                       repadmin.exe is present. Update-NewDcLocatorRecords
#                       re-registers DNS and restarts Netlogon ON the target via
#                       Invoke-Command so its locator (SRV) records publish at
#                       once; Resolve-TopologyDcList scopes the forced KCC by
#                       $ReplicationTopologyScope (Pair / Site / Enterprise) so
#                       a 39-DC estate is not hit with a forest-wide KCC unless
#                       asked; Invoke-Repadmin wraps repadmin.exe (native arg
#                       splatting, no backticks, raw output to the log); and
#                       Write-NewDcReplicationPartner reports the resulting
#                       inbound partners via Get-ADReplicationPartnerMetadata.
#                       The step runs only after a successful, healthy promotion
#                       and is gated by a third Confirm-Step prompt. Honors
#                       $DryRun. No IFM, promotion, privilege or report wiring
#                       changed.
...
$Script:ReplicationPartnerSummary = $null   # one-line outcome of the optional replication speed-up
...
# ----- Replication topology speed-up (optional post-promotion step) -----
# After a successful promotion the KCC normally builds the new DC's connection
# objects on its own ~15-minute cycle. This optional, opt-in step forces that
# work to happen immediately: it re-registers the new DC's locator (SRV) records
# on the target, triggers the KCC, forces an initial sync, then reports the
# inbound partners that resulted. It runs only after a successful, healthy
# promotion and is gated by its own Confirm-Step prompt. Honors $DryRun.
#
# $ReplicationTopologyScope controls how wide the forced KCC / sync reaches:
#   'Pair'       - the new DC and the pinned replication source only (lightest).
#   'Site'       - the new DC and every DC in the new DC's AD site (default).
#   'Enterprise' - the new DC and every DC in the forest (heaviest - adds the
#                  repadmin /e enterprise flag to the sync; this is the
#                  controlled equivalent of "repadmin /kcc *" across all 39 DCs.
#                  Use only when cross-site partners must form immediately.)
$ReplicationTopologyScope    = 'Site'

# Seconds to let DNS registration settle after the Netlogon restart on the new
# DC before the first KCC pass.
$ReplicationDnsSettleSeconds = 15

# Seconds to let the KCC materialize connection objects before forcing the sync.
# The sync requires those connection objects to already exist.
$ReplicationKccSettleSeconds = 20
...
function Build-ReportHtml
{
    ...
    # ----- Replication topology speed-up -----
    $replPartnerText = if (-not [string]::IsNullOrWhiteSpace($Script:ReplicationPartnerSummary)) { $Script:ReplicationPartnerSummary } else { 'Not performed' }

    if ($replPartnerText -like '*present*')
    {
        $replPartnerColor = '#2e7d32'
    }
    elseif (($replPartnerText -like '*No inbound*') -or ($replPartnerText -like '*could not*'))
    {
        $replPartnerColor = '#ef6c00'
    }
    else
    {
        $replPartnerColor = '#555555'
    }
...
<h3 style="border-bottom:2px solid #1565c0;padding-bottom:4px;">Replication Topology Speed-Up</h3>
<table cellpadding="0" cellspacing="0" border="0" style="border-collapse:collapse;">
<tr><td style="padding:4px 10px;border:1px solid #d0d0d0;background:#f4f4f4;font-weight:bold;">Replication partners</td><td style="padding:4px 10px;border:1px solid #d0d0d0;color:$replPartnerColor;font-weight:bold;">$replPartnerText</td></tr>
</table>
...
# ============================================================
# REGION: REPLICATION TOPOLOGY
# ============================================================

function Invoke-Repadmin
{
    # Thin wrapper around repadmin.exe: native-splats the argument array (no
    # backticks), writes raw repadmin output to the log file, and returns the
    # process exit code. repadmin runs locally on this management DC and targets
    # the named DSA directly, so it authenticates in a single hop with the
    # elevated session's own ticket - no double-hop, no credential needed.
    param
    (
        [string[]]$Arguments,
        [string]$Activity
    ...
    Write-InfoMsg "repadmin $($Arguments -join ' ')"

    $output = & repadmin @Arguments 2>&1
    $code   = $LASTEXITCODE
    ...
    if ($code -eq 0)
    {
        Write-SuccessMsg "$Activity completed (repadmin exit 0)."
    }
    else
    {
        Write-WarnMsg "$Activity returned repadmin exit code $code. Review the log for detail."
    }

    return $code
}

function Assert-ReplicationSpeedupReady
{
    # Validates that this session can actually run the speed-up operations:
    #   - repadmin /kcc and /syncall against the new DC and its partners are
    #     forest / domain replication-management operations. The elevated SA
    #     session is already gated by Assert-ExecutionPrivileges (full token +
    #     Domain Admins + Enterprise Admins), which is exactly what they need.
    #   - the remote Netlogon restart on the target uses the validated promotion
    #     credential (a confirmed Enterprise Admin, hence a local admin on the
    #     target DC). If none is available the DNS / SRV re-registration step is
    #     skipped, but the KCC and sync still run.
    #   - repadmin.exe must be present on this management host.
    $ok = $true

    if ($Script:PrivilegesAsserted)
    {
        Write-SuccessMsg 'Privilege check: elevated Domain Admins + Enterprise Admins session - sufficient for repadmin /kcc and /syncall.'
    }
    else
    {
        Write-ErrorMsg 'Execution privileges were not asserted (elevation / Domain Admins / Enterprise Admins); the KCC and syncall calls would be denied.'
        $ok = $false
    }

    if (Get-Command -Name 'repadmin.exe' -ErrorAction SilentlyContinue)
    {
        Write-SuccessMsg 'Tool check: repadmin.exe is available.'
    }
    else
    {
        Write-ErrorMsg "'repadmin.exe' was not found on this host; cannot force the KCC or an initial sync."
        $ok = $false
    }

    if ($null -eq (Get-PromotionCredential))
    {
        Write-WarnMsg 'No promotion credential is available for the remote Netlogon restart; the locator-record re-registration on the target will be skipped (KCC and syncall will still run).'
    }
    else
    {
        Write-SuccessMsg 'Credential check: validated promotion credential available for the remote Netlogon restart on the target.'
    }

    return $ok
}

function Update-NewDcLocatorRecords
{
    # Runs ON the target DC (via Invoke-Command, reusing the validated promotion
    # credential to defeat any double-hop): re-registers host DNS records and
    # restarts Netlogon, which forces the DC's locator (SRV) records to publish
    # immediately rather than on Netlogon's hourly refresh.
    param
    (
        [string]$TargetFqdn
    )

    Write-InfoMsg "Re-registering DNS and restarting Netlogon on '$TargetFqdn' to publish its locator (SRV) records."

    $cred = Get-PromotionCredential

    if ($null -eq $cred)
    {
        Write-WarnMsg 'No credential available; skipping the locator-record re-registration on the target.'
        return
    }
    ...
    try
    {
        ...
        Write-SuccessMsg "DNS re-registered and Netlogon restarted on '$TargetFqdn'."
    }
    catch
    {
        Write-WarnMsg "Could not re-register DNS / restart Netlogon on '$TargetFqdn': $($_.Exception.Message). Continuing - the KCC and sync below will still run."
    }
}

function Resolve-TopologyDcList
{
    # Returns the ordered list of DC FQDNs the KCC should be forced on, based on
    # $ReplicationTopologyScope. The new DC is always first so its inbound
    # connections are built before its partners recompute.
    param
    (
        [string]$NewDcFqdn,
        [string]$NewDcSite
    ...
    switch ($ReplicationTopologyScope)
    {
        'Pair'
        {
            $sourceDc = Get-ADDomainController -Identity $ReplicationSourceDC -Server $ReplicationSourceDC -ErrorAction SilentlyContinue

            if ($sourceDc -and ($sourceDc.HostName -ne $NewDcFqdn))
            {
                $list.Add($sourceDc.HostName)
            }
        }

        'Enterprise'
        {
            $all = Get-ADDomainController -Filter * -Server $ReplicationSourceDC -ErrorAction SilentlyContinue

            foreach ($dc in $all)
            {
                if ($dc.HostName -ne $NewDcFqdn)
                {
                    $list.Add($dc.HostName)
                }
            }
        }
        ...
        default
        {
            # 'Site' - the new DC's own AD site only. If the site is unknown,
            # this collapses to the new DC plus the pinned replication source.
            if ([string]::IsNullOrWhiteSpace($NewDcSite))
            {
                $sourceDc = Get-ADDomainController -Identity $ReplicationSourceDC -Server $ReplicationSourceDC -ErrorAction SilentlyContinue
            ...
            else
            {
                ...
                foreach ($dc in $all)
                {
                    if (($dc.Site -eq $NewDcSite) -and ($dc.HostName -ne $NewDcFqdn))
                    {
                        $list.Add($dc.HostName)
                    }
                }
...
function Write-NewDcReplicationPartner
{
    # Reads the inbound replication partners that now exist for the new DC, logs
    # a per-partition summary, and stores a one-line summary in a script-scope
    # variable for reference in the log.
    param
    (
        [string]$NewDcFqdn
    )
    ...
    try
    {
        $partners = Get-ADReplicationPartnerMetadata -Target $NewDcFqdn -PartnerType Inbound -ErrorAction Stop
    }
    catch
    {
        Write-WarnMsg "Could not read inbound replication partners for '$NewDcFqdn' yet: $($_.Exception.Message)"
        $Script:ReplicationPartnerSummary = 'Inbound partners could not be read immediately after the speed-up.'
        return
    }
    ...
    if ($count -gt 0)
    {
        Write-SuccessMsg "'$NewDcFqdn' now has $count inbound replication partner(s):"

        foreach ($p in $partners)
        {
            $partnerServer = $p.Partner
            $parts         = $partnerServer -split ','

            if ($parts.Count -ge 2)
            {
                $partnerServer = $parts[1] -replace '^CN=', ''
            }

            Write-InfoMsg "    Partition '$($p.Partition)' <- '$partnerServer'"
        }

        $Script:ReplicationPartnerSummary = "$count inbound replication partner(s) present immediately after the speed-up."
    }
    else
    {
        Write-WarnMsg "'$NewDcFqdn' reports no inbound partners yet; the KCC may still be completing. Re-check with 'repadmin /showrepl $NewDcFqdn' shortly."
        $Script:ReplicationPartnerSummary = 'No inbound partners reported yet immediately after the speed-up (KCC may still be completing).'
    }
}

function Invoke-ReplicationTopologySpeedup
{
    # Best-effort orchestration to make the new DC's replication partners
    # (connection objects) appear in minutes instead of on the next KCC cycle:
    #   1) re-register the new DC's locator records (on the target)
    #   2) force the KCC across the scoped DC list (local repadmin)
    #   3) force an initial sync once connection objects exist
    #   4) report the resulting inbound partners
    # Honors $DryRun. Never throws into the caller; degrades to warnings so a
    # successful, healthy promotion is never undone by a topology-acceleration
    # hiccup.
    param
    (
        [hashtable]$TargetInfo
    ...
    Write-InfoMsg '----- Speeding up replication partner creation -----'

    $newDcFqdn = $TargetInfo.HostName
    $newDcSite = if (-not [string]::IsNullOrWhiteSpace($TargetInfo.Site)) { $TargetInfo.Site } else { '' }

    if ($DryRun)
    {
        Write-DryRunMsg "Would re-register DNS and restart Netlogon on '$newDcFqdn'."
        Write-DryRunMsg "Would run 'repadmin /kcc' across scope '$ReplicationTopologyScope' (new DC plus partners)."
        Write-DryRunMsg "Would run 'repadmin /syncall' from '$newDcFqdn'."
        Write-DryRunMsg 'Would report the resulting inbound replication partners.'
        $Script:ReplicationPartnerSummary = 'Dry run - replication topology speed-up not executed.'
        return
    }

    if (-not (Assert-ReplicationSpeedupReady))
    {
        Write-WarnMsg 'Skipping the replication topology speed-up; the KCC will still build the topology on its normal ~15-minute cycle.'
        $Script:ReplicationPartnerSummary = 'Speed-up skipped - prerequisites not met.'
        return
    }

    # Resolve the new DC's AD site from the pinned source if it was not detected
    # earlier in the workflow.
    if ([string]::IsNullOrWhiteSpace($newDcSite))
    {
        try
        {
            $dc        = Get-ADDomainController -Identity $newDcFqdn -Server $ReplicationSourceDC -ErrorAction Stop
            $newDcFqdn = $dc.HostName
            $newDcSite = $dc.Site
        }
        catch
        {
            Write-WarnMsg "Could not resolve the AD site for '$newDcFqdn': $($_.Exception.Message). Scoping the KCC to the new DC and the replication source."
            $newDcSite = ''
        }
    }

    Write-InfoMsg "New DC '$newDcFqdn' in AD site '$(if ($newDcSite) { $newDcSite } else { 'unknown' })'."

    $dcList = Resolve-TopologyDcList -NewDcFqdn $newDcFqdn -NewDcSite $newDcSite
    Write-InfoMsg "Scope '$ReplicationTopologyScope' selected $($dcList.Count) DC(s) for the KCC pass: $($dcList -join ', ')."

    if ($ReplicationTopologyScope -eq 'Enterprise')
    {
        $syncFlags = '/AdeP'
    }
    else
    ...
    # 1) Publish the new DC's locator records now (runs on the target).
    Update-NewDcLocatorRecords -TargetFqdn $newDcFqdn

    Write-InfoMsg "Waiting $ReplicationDnsSettleSeconds second(s) for DNS registration to settle."
    Start-Sleep -Seconds $ReplicationDnsSettleSeconds

    # 2) Force the KCC so connection objects are built immediately.
    foreach ($dc in $dcList)
    {
        $null = Invoke-Repadmin -Arguments @('/kcc', $dc) -Activity "KCC recalculation on '$dc'"
    }

    Write-InfoMsg "Waiting $ReplicationKccSettleSeconds second(s) for the KCC to materialize connection objects."
    Start-Sleep -Seconds $ReplicationKccSettleSeconds

    # 3) Force an initial synchronization now that connection objects exist.
    $null = Invoke-Repadmin -Arguments @('/syncall', $newDcFqdn, $syncFlags) -Activity "syncall from '$newDcFqdn'"

    # 4) Report the inbound partners that now exist.
    Write-NewDcReplicationPartner -NewDcFqdn $newDcFqdn
}
...
function Invoke-PromotionWorkflow
{
    ...
    # ----- Optional: speed up replication partner creation (after a healthy promotion) -----
    $speedupDetail =
    @(
        "New DC : $($targetInfo.HostName)"
        "AD site: $(if ($targetInfo.Site) { $targetInfo.Site } else { 'default' })"
        "Scope  : $ReplicationTopologyScope (re-register locator records, force KCC, force initial sync)"
    )

    if (Confirm-Step -Title 'Speeding up Replication Partner Creation' -Detail $speedupDetail)
    {
        Invoke-ReplicationTopologySpeedup -TargetInfo $targetInfo
    }
    else
    {
        Write-InfoMsg 'Replication topology speed-up declined; the KCC will build the topology on its normal ~15-minute cycle.'
        $Script:ReplicationPartnerSummary = 'Speed-up declined by operator; topology will form on the normal KCC cycle.'
    }
